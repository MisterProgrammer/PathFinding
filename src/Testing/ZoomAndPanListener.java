...
public class ZoomAndPanListener implements MouseListener, MouseMotionListener, MouseWheelListener {
	...
	public static final int DEFAULT_MAX_ZOOM_LEVEL = 9;
...
//        System.out.println9"============= Move camera ============");
		try {
	...
	private void zoomCamera(MouseWheelEvent e) {
		try {
			...
			Point p = e.getPoint();
			if (wheelRotation > 0) {
				if (zoomLevel < DEFAULT_MAX_ZOOM_LEVEL) {
					zoomLevel++;
					Point2D p1 = transformPoint(p);
					coordTransform.scale(1 / DEFAULT_ZOOM_MULTIPLICATION_FACTOR,
							1 / DEFAULT_ZOOM_MULTIPLICATION_FACTOR);
					Point2D p2 = transformPoint(p);
					coordTransform.translate(p2.getX() - p1.getX(), p2.getY() - p1.getY());
					targetComponent.repaint();
				}
			} else {
				if (zoomLevel > DEFAULT_MIN_ZOOM_LEVEL) {
					zoomLevel--;
					Point2D p1 = transformPoint(p);
					coordTransform.scale(DEFAULT_ZOOM_MULTIPLICATION_FACTOR, DEFAULT_ZOOM_MULTIPLICATION_FACTOR);
					Point2D p2 = transformPoint(p);
					coordTransform.translate(p2.getX() - p1.getX(), p2.getY() - p1.getY());
					targetComponent.repaint();
				}
